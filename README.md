...
| **Infrastructure** | `routes [term] [--module MODULE] [--include-tests] [--limit N] [--cursor CURSOR]`, `sql`, `envs`, `errors`, `concurrency`, `globals`, `httpcalls`, `deps [--transitive]`, `dependents`, `imports` | Bounded, cursor-continuable HTTP route pages with CLI/MCP pagination parity and root/nested module selectors (production-only by default; including constant nested Gin/Echo/Fiber groups and Chi Route closures), SQL, env vars, concurrency, outbound HTTP calls, imports. |
