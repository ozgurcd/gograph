...
- **Operational Memory Policy:** Low-memory execution may change garbage-collection cadence, phase scheduling, and temporary representation, but it must not change graph precision or silently omit facts. `--max-memory` is a soft Go heap target rather than an RSS guarantee and must remain explicit in both CLI build and MCP refresh configuration/capabilities.
