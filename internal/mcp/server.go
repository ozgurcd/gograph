...
func NewServer(
	...
	addTool(capabilitiesTool, func(_ context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		resp := map[string]any{
			...
			"prerequisite": "The MCP server loads .gograph/graph.json when present and auto-builds an in-memory graph when it is missing. Source-analysis tools check freshness per call, preserve the latest graph while source is unchanged, and rebuild in memory after edits; gograph_stale, gograph_changes without git_ref, and gograph_stats inspect the persisted index. Run `gograph build . --precise` for type-checked CHA/SSA enrichment; MCP preserves it until source changes. Session lifecycle tools write local telemetry, gograph_session_cleanup deletes stale logs, gograph_boundaries_create writes configuration, and gograph_wiki writes documentation. gograph_doc invokes the local Go toolchain and is annotated open-world because module resolution follows the user's Go environment.",
			"tools": []map[string]string{
				...
				{"name": "gograph_stats", "purpose": "Repository-level statistics plus complete/partial build status, parsed/scanned file counts, and parse failures."},
	...
	callersTool := mcp.NewTool("gograph_callers",
		mcp.WithDescription("Find functions and methods that call the specified function. Defaults to one-hop fan-in; depth 2-10 expands callers-of-callers. The MCP server refreshes source analysis before the call. Read-only; no persistent side effects. WHEN TO USE: Before renaming, removing, or changing a function signature. NOT TO USE: For unbounded upstream blast radius (use gograph_impact); for downstream callees (use gograph_callees). RETURNS: Caller symbols with package paths, file locations, and call-site line numbers."),
	...
	orphansTool := mcp.NewTool("gograph_orphans",
		mcp.WithDescription("Find functions and methods unreachable from runtime roots (main/init), test/benchmark/fuzz roots, HTTP route handlers, and eligible externally callable exports; exports confined under internal/ are not roots. Uses full BFS reachability. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. WHEN TO USE: During code cleanup or dead-code audits. NOT TO USE: For checking one symbol's usages (use gograph_usages or gograph_callers). RETURNS: Orphan symbols with package paths and file locations; empty means no unreachable code was detected."),
	)
	...
	impactTool := mcp.NewTool("gograph_impact",
		mcp.WithDescription("Traverse the call graph backwards to find every symbol that transitively calls the target — the full upstream blast radius of a change. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. Three modes: (1) single symbol via `symbol`; (2) uncommitted-changes blast radius via `uncommitted=true`; (3) git-ref changes blast radius via `since`. WHEN TO USE: Before refactoring a core function to see what breaks; use uncommitted=true after editing to verify scope. NOT TO USE: For direct one-hop callers only (use gograph_callers instead). RETURNS: Transitive list of upstream affected symbols; JSON with count:0 message when no symbols are modified or no callers found."),
	...
	endpointTool := mcp.NewTool("gograph_endpoint",
		mcp.WithDescription("Build a full vertical slice for one HTTP route: the matched handler symbol, a BFS call chain downstream (default depth 5), all SQL queries emitted in that chain, and all env vars read. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. LIMITATION: Nested route-group prefixes (e.g. Gin/Echo/Chi Group()) are lost at the static AST level. Always query by the final route path suffix or, ideally, by the handler function symbol name (e.g., 'CreateUser'). WHEN TO USE: When auditing what an API endpoint does end-to-end — its downstream dependencies, database queries, and configuration reads. NOT TO USE: For listing all routes (use gograph_routes first to find the pattern); for raw handler source code only (use gograph_source). RETURNS: Array of endpoint slices with route, handler, call chain, SQL, and env fields; found:false with a suggestion when the query does not match any route. `query` accepts route pattern (\"POST /api/users\"), path fragment (\"/users\"), or handler name. `depth` controls call-chain BFS depth (default: 5)."),
	...
	routesTool := mcp.NewTool("gograph_routes",
		mcp.WithDescription("List all HTTP routes registered in the codebase with their HTTP methods, URL patterns, and handler function names. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. LIMITATION: Nested route-group prefixes (e.g. Gin/Echo/Chi Group()) are lost at the static AST level. Routes are recorded under their final path suffix (e.g., '/users' instead of '/api/v1/users'). WHEN TO USE: To get the complete API surface of a service before deep-diving into a specific route with gograph_endpoint. NOT TO USE: For full call chain analysis of a route (use gograph_endpoint instead). RETURNS: Structured table of method/path/handler triples; empty when no HTTP routes are registered in the graph."),
	)
	...
	contextTool := mcp.NewTool("gograph_context",
		mcp.WithDescription("Fetch a pre-flight context bundle for a single Go symbol: AST node metadata, source code, direct callers, direct callees, linked test functions, and architectural role classification — all in one call. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. Set uncommitted=true to bundle context for all currently modified symbols at once. WHEN TO USE: As the first call before editing a symbol — eliminates 4–5 separate tool roundtrips. NOT TO USE: For package-level orientation (use gograph_focus); for transitive blast radius (use gograph_impact). RETURNS: JSON with node, source, callers[], callees[], tests[], and role; empty object {} when symbol not found. With uncommitted=true, returns a contexts[] array; count:0 when no uncommitted symbols exist."),
	...
	reviewTool := mcp.NewTool("gograph_review",
		mcp.WithDescription("Summarize the scope and risk profile of a change: which symbols changed, which tests cover them, which routes and env vars they touch, and whether SQL is involved. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. Requires either symbol or uncommitted=true. WHEN TO USE: After editing — as a post-edit verification step before committing; confirms the blast radius matches expectations. Use uncommitted=true to review all current unstaged changes at once. NOT TO USE: For boundary constraint enforcement (use gograph_boundaries); for pre-edit planning (use gograph_plan). RETURNS: JSON with changed_symbols[], tests[], routes[], env[], errors[], and a risk object (public_api, touches_sql, touches_routes, touches_env)."),
	...
	errorsTool := mcp.NewTool("gograph_errors",
		mcp.WithDescription("Find all error creation sites in the codebase: errors.New, fmt.Errorf, and sentinel var declarations. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. Optional `term` filters by error message substring (e.g., \"ErrInvalid\", \"unauthorized\"). WHEN TO USE: When cataloging error codes, standardizing error messages, or checking whether a specific error string is already defined before adding a new one. NOT TO USE: For tracing how an error propagates up the call stack (use gograph_errorflow instead). RETURNS: List of error creation sites with message text, file path, and line number; empty when no matches found."),
...
func initNewTools(
	...
	concurrencyTool := mcp.NewTool("gograph_concurrency",
		mcp.WithDescription("Find all concurrency primitives in the codebase: goroutine spawns (`go` statements), channel operations, sync.Mutex/RWMutex, sync.WaitGroup, sync.Once, and select statements. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. Optional `term` filter (e.g., \"mutex\", \"goroutine\", \"channel\"). WHEN TO USE: When auditing race safety, understanding async flow, or locating all synchronization points before a concurrency refactor. NOT TO USE: For standard sequential call flow analysis (use gograph_callers/gograph_callees). RETURNS: File locations, line numbers, and primitive kind for each concurrency site; empty when no concurrency primitives are found."),
	...
	godobjTool := mcp.NewTool("gograph_godobj",
		mcp.WithDescription("Detect God Object anti-pattern candidates by scoring structs on method count, field count, and outgoing call count. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. Thresholds: `methods` (default: 5), `fields` (default: 8), `calls` (default: 15); `top` limits results (default: 10). Exceeding any enabled threshold qualifies a struct, and the combined excess determines rank. WHEN TO USE: During architecture reviews to find monolithic structs that should be decomposed. NOT TO USE: For general struct layout inspection (use gograph_fields); for single-function complexity (use gograph_complexity). RETURNS: Ranked candidates with method, field, and call counts; empty when no threshold is exceeded."),
	...
	skeletonTool := mcp.NewTool("gograph_skeleton",
		mcp.WithDescription("Emit the full repository's API signatures with function bodies stripped — struct definitions, interface declarations, and function/method signatures only. The MCP server checks freshness before this call and rebuilds its in-memory AST graph only when source changed; run `gograph build . --precise` first when type-checked CHA/SSA enrichment is required. Read-only; no side effects. WARNING: output can be very large on big repositories — consider using gograph_public per package for targeted queries. WHEN TO USE: When an LLM needs a compact map of the entire codebase's shape without reading source files individually. NOT TO USE: For full implementations (use gograph_source); for a single package (use gograph_public). RETURNS: Multi-line text of all stripped declarations across all packages; always non-empty when the graph has symbols."),
	)
